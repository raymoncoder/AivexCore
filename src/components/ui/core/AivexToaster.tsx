...
type ToastType = "success" | "error" | "warning" | "info" | "Aivex";
...
export const AivexToastProvider = ({ children }: { children: React.ReactNode }) => {
    const [toasts, setToasts] = useState<Toast[]>([]);

    const toast = useCallback((message: string, options?: { type?: ToastType; description?: string; duration?: number }) => {
        const id = Math.random().toString(36).substring(2, 9);
        const { type = "Aivex", description, duration = 5000 } = options || {};
                        ...
                        >
                            ...
                            <div className="flex gap-4">
                                <div className={cn(
                                    ...
                                    t.type === "Aivex" && "text-zinc-400"
                                )}>
                                    {t.type === "success" && <CheckCircle size={18} />}
                                    {t.type === "error" && <AlertOctagon size={18} />}
                                    {t.type === "warning" && <AlertTriangle size={18} />}
                                    {t.type === "info" && <Info size={18} />}
                                    {t.type === "Aivex" && <div className="w-4 h-4 rounded-full border-2 border-current animate-pulse" />}
                                </div>
...
export const useAivexToast = () => {
    const context = useContext(ToastContext);
    if (!context) throw new Error("useAivexToast must be used within AivexToastProvider");
    return context;
};
